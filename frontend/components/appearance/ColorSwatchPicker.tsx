  ...
  return (
    <div className="mt-1 flex flex-wrap items-center gap-1.5">
      ...
      <button
        ...
        className={`relative h-7 w-7 shrink-0 rounded-md border-2 transition ${
          !matchesPreset
            ? "border-black ring-2 ring-black/30 ring-offset-1"
            : "border-zinc-300 hover:border-zinc-500"
        }`}
      ...
      >
        <input
          ...
          className="absolute inset-0 h-full w-full cursor-pointer opacity-0 rounded-md"
          aria-hidden="true"
          tabIndex={-1}
        />

        {/* Always-visible badge so this swatch reads as "pick a custom
            color" whether it's showing the rainbow icon or a solid color
            the user already picked. Sized up from the original so it
            stands out, but kept smaller than the swatch itself so the
            color/rainbow preview underneath is still visible. */}
        <span
          aria-hidden="true"
          className={`pointer-events-none absolute -bottom-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full border-2 border-black/30 text-sm font-black leading-none shadow-sm ${
            matchesPreset ? "bg-white text-black" : "bg-white/95 text-black"
          }`}
        >
          +
        </span>
